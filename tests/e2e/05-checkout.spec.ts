...
test("paiement Stripe test → confirmation → commande payée", async ({
  ...
  // Formulaire : livraison (+9€) → total 101€
  ...
  await page.getByText("Livraison à vélo").click();
  await page
    .getByLabel("Adresse de livraison *")
    .fill("3 quai Valin, 17000 La Rochelle");
  await page
    .getByLabel("Message pour la carte", { exact: false })
    .fill("Pour les tests, avec amour.");
  await expect(page.getByTestId("checkout-total")).toHaveText("101€");
  ...
  await expect(confirmation).toContainText("101€");
...
test("la commande payée apparaît dans l'admin et suit ses statuts", async ({
  ...
  await expect(row).toContainText("Livraison");
  ...
  await expect(
    page.getByText("Statut final — plus aucune transition possible."),
  ).toBeVisible();
});
...
test("le dashboard reflète la vente dans les KPIs", async ({ page }) => {
  ...
  await expect(page.getByTestId("kpi-revenue")).toHaveText("101€");
  await expect(page.getByTestId("kpi-orders")).toHaveText("1");
  await expect(page.getByTestId("kpi-aov")).toHaveText("101€");
